...
const calculateStudentScore = (student: HonoredStudent): number => {
    return Object.values(student.votes || {}).reduce((total, vote) => total + vote.criteriaKeys.length, 0);
};
...
export default function HonorBoardView({ currentUser, classes }: HonorBoardViewProps) {
    ...
    const principalId = currentUser.role === 'student' ? currentUser.principalId : (currentUser.role === 'principal' ? currentUser.id : currentUser.principalId);
    ...
    const topStudentsByStage = useMemo(() => {
        ...
        Object.entries(boardData).forEach(([stage, board]) => {
             ...
             const studentsWithScores = Object.values(board.honoredStudents)
