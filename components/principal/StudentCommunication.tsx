...
// ===================================
const ChatWindow = ({ conversation, currentUser, messages, classes }: { conversation: Conversation, currentUser: User, messages: ChatMessage[], classes: ClassData[] }) => {
    ...
    const handleSendMessage = async () => {
        ...
        try {
            if (attachment) {
                ...
                if (isImage) {
                    try {
                        ...
                        console.warn("Could not compress image, uploading original.", compError);
                    }
                }
            ...
            // Use set to create the conversation if it doesn't exist yet
                        ...
                        if (student.studentAccessCode) {
                            ...
                            if (newNotifKey) {
                                const notification: Omit<StudentNotification, 'id'> = {
                                    ...
                                    message: `رسالة جديدة في محادثة المجموعة "${conversation.groupName}" من ${currentUser.name}.`,
    ...
    return (
        <div className="flex flex-col h-full bg-white border rounded-lg shadow-inner">
                             ...
                             {msg.attachment && (
                                 <div className="mt-2">
                                     {msg.attachment.type === 'image' ? (
                                        ...
                                        <a href={msg.attachment.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-2 bg-black/20 rounded-lg hover:bg-black/30">
                                            <FileIcon size={20}/>
            ...
            <div className="p-2 border-t bg-white">
                {attachment && (
                    <div className="p-2 flex items-center justify-between bg-gray-100 rounded-md mb-2">
                        <span className="text-sm truncate">{attachment.name}</span>
                        <button onClick={() => { setAttachment(null); if(fileInputRef.current) fileInputRef.current.value = ''; }} className="p-1 text-red-500 hover:bg-red-100 rounded-full"><X size={16}/></button>
                    </div>
                )}
                <div className="flex items-center gap-2">
                    ...
                    <button onClick={handleSendMessage} disabled={isSending} className="p-3 bg-cyan-600 text-white rounded-md hover:bg-cyan-700 disabled:bg-gray-400">
...
export default function StudentCommunication({ principal, settings, classes }: StudentCommunicationProps) {
    ...
    const [newChatSelection, setNewChatSelection] = useState({ stage: '', classId: '' });
    
    useEffect(() => {
        const convRef = db.ref(`conversations/${principal.id}`);
        const callback = (snapshot: any) => {
            const data = snapshot.val() || {};
            const convList: Conversation[] = Object.values(data);
            setConversations(convList.filter(c => !c.teacherId).sort((a, b) => b.lastMessageTimestamp - a.lastMessageTimestamp));
        };
        ...
        if (activeConversation) {
            ...
            const callback = (snapshot: any) => {
                ...
                if (activeConversation.unreadByStaff) {
                     db.ref(`conversations/${principal.id}/${activeConversation.id}/unreadByStaff`).set(false);
                }
            };
    ...
    }, [activeConversation, principal.id]);

    const availableStages = useMemo(() => Array.from(new Set(classes.map(c => c.stage))), [classes]);
    
    const classesForStage = useMemo(() => {
        if (!newChatSelection.stage) return [];
        return classes.filter(c => c.stage === newChatSelection.stage);
    }, [classes, newChatSelection.stage]);

    const studentsForNewChat = useMemo(() => {
        if (!newChatSelection.classId) return [];
        const classData = classes.find(c => c.id === newChatSelection.classId);
        return (classData?.students || [])
            .filter(s => s.studentAccessCode)
            .sort((a,b) => a.name.localeCompare(b.name, 'ar'));
    }, [classes, newChatSelection.classId]);
    ...
    const handleStartNewChat = (student: Student) => {
        const convId = `p_${principal.id}__s_${student.id}`;
     ...
     const handleStartGroupChat = (classData: ClassData) => {
        const convId = `group_p_${principal.id}_c_${classData.id}`;
        const existingConv = conversations.find(c => c.id === convId);
        ...
        if (existingConv) {
            ...
            const newConv: Conversation = {
                ...
                groupName: `مجموعة ${classData.stage} / ${classData.section}`,
                staffName: principal.name,
                lastMessageText: '',
                lastMessageTimestamp: Date.now(),
                unreadByStaff: false,
    ...
    const renderRightPanel = () => {
        if (activeConversation) {
            return <ChatWindow conversation={activeConversation} currentUser={principal} messages={messages} classes={classes}/>;
        }
            ...
            return (
                <div className="p-4 bg-white border rounded-lg h-full flex flex-col">
                    ...
                    <div className="space-y-4">
                        <div>
                            <label className="font-semibold block mb-1">1. اختر المرحلة:</label>
                            <select value={newChatSelection.stage} onChange={e => setNewChatSelection({ stage: e.target.value, classId: '' })} className="w-full p-2 border rounded-md bg-white">
                                <option value="">-- اختر المرحلة --</option>
                                {availableStages.map(stage => <option key={stage} value={stage}>{stage}</option>)}
                            </select>
                        </div>
                        {newChatSelection.stage && (
                            <div>
                                <label className="font-semibold block mb-1">2. اختر الشعبة لبدء المحادثة:</label>
                                <div className="max-h-96 overflow-y-auto space-y-1">
                                    {classesForStage.map(c => 
                                        <button key={c.id} onClick={() => handleStartGroupChat(c)} className="w-full text-right p-3 rounded-lg hover:bg-cyan-100 transition-colors">
                                            {c.stage} / {c.section}
                                        </button>
                                    )}
                                </div>
             ...
             return (
                <div className="p-4 bg-white border rounded-lg h-full flex flex-col">
                    ...
                    <div className="space-y-4 mb-2">
                        ...
                        <div>
                            <label className="font-semibold block mb-1">2. اختر الشعبة:</label>
                             <select value={newChatSelection.classId} onChange={e => setNewChatSelection(p => ({...p, classId: e.target.value}))} disabled={!newChatSelection.stage} className="w-full p-2 border rounded-md bg-white disabled:bg-gray-100">
                                <option value="">-- اختر الشعبة لعرض الطلاب --</option>
                                {classesForStage.map(c => <option key={c.id} value={c.id}>{c.section}</option>)}
                            </select>
                        </div>
                    </div>
                    {newChatSelection.classId && (
                        <div className="flex-1 overflow-y-auto border-t pt-2">
                             <label className="font-semibold block mb-1">3. اختر الطالب:</label>
                            {studentsForNewChat.length > 0 ? (
                                studentsForNewChat.map(student => (
                                    <button key={student.id} onClick={() => handleStartNewChat(student)} className="w-full text-right p-2 rounded-md hover:bg-gray-100 font-semibold">{student.name}</button>
                                ))
        ...
        return (
            <div className="flex flex-col items-center justify-center h-full bg-gray-50 border rounded-lg text-gray-500 text-center p-4">
                ...
                <p className="text-sm mt-2">أو انقر على أحد الأزرار في الأعلى لبدء محادثة جديدة.</p>
            </div>
        );
    }
            ...
            <div className="md:col-span-1 bg-white border rounded-lg p-2 flex flex-col">
                <div className="p-2 border-b space-y-2">
                    <h3 className="font-bold text-lg text-center">إنشاء محادثة</h3>
                     <div className="flex gap-2">
                        ...
                        >
                            <Users size={18}/> إجراء محادثة جماعية
                        </button>
                        <button 
                            onClick={() => { setViewMode('new_individual'); setActiveConversation(null); }} 
                        ...
                        >
                            <UserPlus size={18}/> إجراء محادثة فردية
                        </button>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto">
                    {conversations.map(conv => {
                        ...
                        return (
                            <button key={conv.id} onClick={() => handleSelectConversation(conv)} className={`w-full text-right p-3 rounded-md flex justify-between items-center ${isActive ? 'bg-cyan-500 text-white' : 'hover:bg-gray-100'}`}>
                                <div className="flex items-center gap-2 overflow-hidden">
                                    ...
                                    <div className="truncate">
                                        ...
                                        <p className={`text-sm truncate ${isActive ? 'text-cyan-100' : 'text-gray-500'}`}>{conv.lastMessageText}</p>
                                    </div>
                                </div>
