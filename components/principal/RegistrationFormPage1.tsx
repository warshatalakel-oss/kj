...
export default function RegistrationFormPage1({ formData, onUpdate, studentPhoto, onPhotoUpload, isPdfMode = false }: RegistrationFormPage1Props) {
    ...
    return (
        <div className="w-[850px] mx-auto p-4 bg-[#fcf3d9] border-4 border-yellow-400 rounded-2xl" style={{ fontFamily: "'Asir', sans-serif" }}>
            ...
            <div className="grid grid-cols-12 gap-x-4 gap-y-10">
                {/* Photo and ID */}
                <div className="col-span-3 flex flex-col items-center space-y-8">
                    <div
                        onClick={() => !isPdfMode && fileInputRef.current?.click()}
                        className={`relative w-32 h-40 bg-white border-2 border-cyan-800 rounded-2xl flex items-center justify-center text-gray-400 ${!isPdfMode ? 'cursor-pointer hover:bg-gray-100' : ''}`}
                    >
                        {studentPhoto ? (
                            <img src={studentPhoto} alt="صورة الطالب" className="w-full h-full object-cover rounded-2xl" />
                        ) : (
                            <div className="text-center">
                                <Camera size={48} />
                                {!isPdfMode && <p className="text-xs mt-1">انقر للتحميل</p>}
                            </div>
                        )}
                         <div className="absolute -top-6 left-1/2 -translate-x-1/2 px-3 py-0.5 bg-[#f5d47a] border-2 border-cyan-800 rounded-full text-center text-lg font-bold whitespace-nowrap">
                            <span style={{ position: 'relative', top: '-8px' }}>صورة الطالب</span>
                        </div>
                    </div>
                     <input type="file" ref={fileInputRef} onChange={onPhotoUpload} accept="image/*" className="hidden" disabled={isPdfMode} />
